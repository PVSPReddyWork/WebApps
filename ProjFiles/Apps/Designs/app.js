...
// Function to set the image with tiling/cropping or fit-to-size
function applyImageToRectangle(rectangle, imageURL, fitToSize) {
  if (fitToSize) {
    // Fit the image to the rectangle
    rectangle.style.backgroundImage = `url(${imageURL})`;
    rectangle.style.backgroundSize = "100% 100%"; // Stretch to fit
    rectangle.style.backgroundRepeat = "no-repeat";
  } else {
    // Tile/crop the image
    rectangle.style.backgroundImage = `url(${imageURL})`;
    rectangle.style.backgroundSize = "auto"; // No scaling
    rectangle.style.backgroundRepeat = "repeat"; // Enable tiling
  }
}
...
//addImageButton.onclick = () => 
  ...
  if (selectedRectangle) {
    ...
    inputFile.onchange = (event) => {
      ...
      const fitToSize = fitToSizeCheckbox.checked;
      if (selectedRectangle) {
        applyImageToRectangle(selectedRectangle, imageURL, fitToSize);
      }
          /** /
          if (fitToSize) {
            // Fit the image to the rectangle
  ...
  elements.forEach((el) => {
    ...
    if (
      ...
      if (bgImage && bgImage !== "none") {
        ...
        img.onload = () => {
          const fitToSize = el.dataset.fitToSize === "true"; // Check dataset
          if (fitToSize) {
            // Fit image to rectangle
            ctx.drawImage(
              img,
              0,
              0,
...
const screenshotButton1 = document.createElement("button");
screenshotButton1.onclick = async () => {
  const rect = drawingArea.getBoundingClientRect();
  //const rect = selectionTool.getBoundingClientRect();
